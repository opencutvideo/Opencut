...
export default function Home() {
  ...
  return (
    <>
      ...
      >
        {/* Background — layered glows for depth */}
        <div aria-hidden="true" className="pointer-events-none absolute inset-0"
          style={{ background: 'radial-gradient(ellipse 80% 50% at 50% 0%, rgba(255,255,255,0.06) 0%, transparent 70%)' }}
        />
        <div aria-hidden="true" className="pointer-events-none absolute inset-0"
          style={{ background: 'radial-gradient(ellipse 40% 30% at 50% 60%, rgba(255,255,255,0.025) 0%, transparent 100%)' }}
        />
        {/* Dot grid */}
        ...
        {/* Status pill */}
        <div className={`mb-8 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-4 py-1.5 text-sm text-muted-foreground transition-all duration-700 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-3'}`}>
          <span className="h-1.5 w-1.5 rounded-full bg-green-400 animate-pulse" />
          Alpha · Rewrite in progress
        ...
        {/* Headline — tight, confident, no awkward line breaks */}
        <div className={`mb-6 transition-all duration-700 delay-100 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <h1 className="font-bold tracking-tight leading-[1.04]">
            <span className="block text-4xl sm:text-5xl md:text-6xl lg:text-[70px] text-white">
              Video editing that
            </span>
            <span className="block text-4xl sm:text-5xl md:text-6xl lg:text-[70px] mt-1">
              <span className="relative inline-block">
                {/* Strikethrough on "costs you" to visually say "doesn't cost you" */}
                <span className="text-white/25 line-through decoration-white/20 decoration-2">costs you.</span>
              </span>
              <span className="text-white"> Doesn't.</span>
            </span>
          </h1>
        </div>

        {/* Feature chips */}
        <div className={`flex flex-wrap justify-center gap-2 mb-7 transition-all duration-700 delay-200 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
          {[
            { icon: '⚡', label: 'Rust + WASM engine' },
            { icon: '🔒', label: 'Runs 100% locally' },
            { icon: '∞', label: 'No export limits' },
            { icon: '🌐', label: 'Web · Desktop · Mobile' },
          ].map((chip) => (
            <span key={chip.label} className="inline-flex items-center gap-1.5 rounded-full border border-white/8 bg-white/[0.03] px-3 py-1 text-xs text-white/55">
              <span>{chip.icon}</span>
              {chip.label}
            </span>
          ))}
        ...
        <p className={`text-base sm:text-lg text-muted-foreground max-w-sm mb-8 leading-relaxed transition-all duration-700 delay-[250ms] ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          Free. Open source. Your footage never leaves your device.
        </p>

        {/* CTAs */}
        <div className={`flex flex-col sm:flex-row items-center gap-3 transition-all duration-700 delay-300 ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          <Link
            to="/editor"
            className="inline-flex items-center justify-center gap-2 bg-white text-black font-semibold px-8 py-3.5 rounded-xl hover:bg-white/90 active:scale-[0.97] transition-all duration-150 shadow-[0_0_40px_rgba(255,255,255,0.12)] text-[15px] w-full sm:w-auto"
          >
          ...
          <a
            ...
            className="inline-flex items-center justify-center gap-2 border border-white/10 text-white/60 font-medium px-7 py-3.5 rounded-xl hover:border-white/20 hover:text-white/85 hover:bg-white/[0.03] active:scale-[0.97] transition-all duration-150 text-[15px] w-full sm:w-auto"
          >
        ...
        {/* Store badges */}
        <div className={`mt-6 mb-12 flex flex-col items-center gap-3 transition-all duration-700 delay-[380ms] ${heroSection.inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
          <p className="text-[11px] text-white/30 uppercase tracking-widest">Also coming to</p>
          <div className="flex items-center gap-3 flex-wrap justify-center">
            ...
            <div className="relative group cursor-not-allowed">
              <img
                ...
                height={42}
                className="h-[42px] w-auto opacity-45 group-hover:opacity-60 transition-opacity duration-200 select-none"
                draggable={false}
              />
              <span className="absolute -top-2 -right-2 bg-white/10 border border-white/15 text-white/70 text-[9px] font-semibold uppercase tracking-wide px-1.5 py-0.5 rounded-full">
                Soon
              </span>
            </div>
            {/* Play Store badge */}
